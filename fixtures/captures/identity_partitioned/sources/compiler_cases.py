...
def main() -> None:
    ...
    parser.add_argument("case", choices=["stable", "shape", "scalar", "multiple_guards", "dynamic", "grad",
                                         "graph_break", "backend_failure", "no_graph", "limit", "identity", "identity_partitioned"])
    ...
    sequences = {
        ...
        "identity_partitioned": [(4, 2, False)] * 3,
    }
    ...
    modules = [IdentityModule() for _ in range(3)] if args.case.startswith("identity") else []
    if args.case == "identity_partitioned":
