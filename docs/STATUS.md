...
2. Finish the remaining native candidate-source/evidence walkthrough once desktop automation provides consistent current state. The September 7 follow-up visually verified the baseline source at line 9 and the rendered comparison table in one editor group, including separate guard columns, counts, unmatched helper and limitations. Opening `payload-10` changed the window title while accessibility content/screenshots remained on the table, so evidence-view acceptance remains unverified. The corrected [CLI demonstration](DEMO.md) is recorded and reproducible. The private repository is uploaded and [hosted CI passed](../artifacts/HOSTED_CI.md).
