...
These instructions create installable artifacts. Python analyzer 0.1.0 uses [GitHub Releases](https://github.com/Arnavsharma2/dynamo-diff/releases/tag/v0.1.0); VS Code extension 0.1.2 is published on [Marketplace](https://marketplace.visualstudio.com/items?itemName=dynamo-diff.dynamo-diff). [Installation instructions](INSTALL.md) cover both components. Python analyzer 0.1.1 is [published on PyPI](https://pypi.org/project/dynamo-diff/); its [publishing workflow and account configuration](PYPI_PUBLISHING.md) and [verified publication receipt](../artifacts/pypi-publication-0.1.1.json) are documented separately.
