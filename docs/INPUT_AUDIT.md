...
The source-map correction changes validation of malformed input; it does not change valid-capture normalization or the recorded pilot's evaluated code. The completed pilot and its negative primary result remain intact. Required human fixture/case/evaluation review is still recorded separately until the user explicitly changes that scope requirement or a review actually occurs.
