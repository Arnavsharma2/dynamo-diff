...
def score(result: dict, expected: dict, transcript: list[dict]) -> dict:
    ...
    return {"id": result["id"], "question_id": result["question_id"], "condition": result["condition"],
        ...
        "raw_final_text": result.get("final_text"),
...
def report(run: Path, output: Path) -> dict:
    ...
    result = {"generated_at": datetime.now(timezone.utc).isoformat(), "score_version": "1.1",
    ...
    for row in rows:
        review += [f"## {row['id']}", "", f"Status: {row['status']}. Correct fields: {row['correct_fields']}/{row['total_fields']}. "
            ...
            code({"answer": row["answer"], "explanation": row["explanation"]}), "", "Original final text (including malformed responses):",
            code(row["raw_final_text"]), "", "Citation checks:", code(row["citations"]), "",
